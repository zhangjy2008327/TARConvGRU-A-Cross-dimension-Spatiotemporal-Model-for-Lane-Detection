...
class gated_resnet(nn.Module):
    ...
    """
    def __init__(self, num_filters, kernel_size, padding, nonlinearity=nn.ReLU, dropout=0.2, dilated=1,batchNormObject=nn.BatchNorm2d):
        super(gated_resnet, self).__init__()
        self.gated = True
        num_hidden_filters =2 * num_filters if self.gated else num_filters
        self.conv_input = nn.Conv2d(num_filters, num_hidden_filters, kernel_size=kernel_size,stride=1,padding=padding,dilation=dilated)
        self.dropout = nn.Dropout2d(dropout)
        self.nonlinearity = nonlinearity()
        self.batch_norm1 = batchNormObject(num_hidden_filters)
        self.conv_out = nn.Conv2d(num_hidden_filters, num_hidden_filters, kernel_size=kernel_size,stride=1,padding=padding,dilation=dilated)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, num_filters, kernel_size, padding, nonlinearity=nn.ReLU, dropout=0.2, dilated=1,batchNormObject=nn.BatchNorm2d):
        ...
        self.conv1 = nn.Conv2d(num_filters, num_hidden_filters, kernel_size=kernel_size,stride=1,padding=padding,dilation=dilated )
        self.dropout = nn.Dropout2d(dropout)
        self.nonlinearity = nonlinearity(inplace=False)
        self.batch_norm1 = batchNormObject(num_hidden_filters)
        self.conv2 = nn.Conv2d(num_hidden_filters, num_hidden_filters, kernel_size=kernel_size,stride=1,padding=padding,dilation=dilated )
        self.batch_norm2 = batchNormObject(num_filters)
...
class ConvolutionalEncoder(nn.Module):
    ...
    def __init__(self,n_features_input,num_hidden_features,kernel_size,padding,n_resblocks,dropout_min=0,dropout_max=0.2, blockObject=ResidualBlock,batchNormObject=nn.BatchNorm2d):
        ...
        for _ in range(n_resblocks):
            ...
            block += [blockObject(num_hidden_features[0], kernel_size, padding, dropout=p,batchNormObject=batchNormObject, dilated=1)]
            # print('convolutonalencoder------1------------')
        ...
        for features_in,features_out in [num_hidden_features[i:i+2] for i in range(0,len(num_hidden_features), 1)][:-1]:
            ...
            p = next(iter(dropout))
            for _ in range(n_resblocks):
                block += [blockObject(features_out, kernel_size, padding, dropout=p,batchNormObject=batchNormObject, dilated=1)]
            self.stages.append(nn.Sequential(*block)) 
...
class UNet(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, out_channels=2, num_hidden_features=[32,32,32],n_resblocks=1,num_dilated_convs=2,
                 dropout_min=0.01, dropout_max=0.3, gated=False, padding=1, kernel_size=3,group_norm=4, blockObject = 'non_bottleneck_1d'):
        """
        ...
        if blockObject == 'gated_resnet':
            self.blockObject = gated_resnet
        batchNormObject = lambda n_features : nn.GroupNorm(group_norm,n_features) if group_norm > 0 else nn.BatchNorm2d
        self.encoder = ConvolutionalEncoder(in_channels,num_hidden_features,kernel_size,padding,n_resblocks,dropout_min=dropout_min,dropout_max=dropout_max,blockObject=self.blockObject,batchNormObject=batchNormObject)
        ...
        self.decoder = ConvolutionalDecoder(out_channels,num_hidden_features[::-1],kernel_size,padding,n_resblocks,dropout_min=dropout_min,dropout_max=dropout_max,blockObject=self.blockObject,batchNormObject=batchNormObject)
        self.convgru = ConvGRU(input_size=(16, 32),
                                 input_dim=32,
                                 hidden_dim=[32, 32],
                                 kernel_size=(3, 3),
                                 num_layers=1,
    ...
    def forward(self, input):
        ...
        output10, _ = self.convgru(data)
        test = output10[0][-1, :, :, :, :]
        x = self.decoder(test,skips[:-1][::-1])
        return x